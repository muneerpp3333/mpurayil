...
function validate(filepath) {
  ...
  // Excerpt length (also serves as meta description — 155 chars optimal for Google)
  ...
  if (data.excerpt && data.excerpt.length > 160) {
    warnings.push(`Excerpt is ${data.excerpt.length} chars — Google truncates meta descriptions after ~155 chars`);
  }

  // SEO: coverImage with alt text
  if (data.coverImage && !data.coverAlt) {
    warnings.push('coverImage set without coverAlt — add alt text for accessibility and SEO');
  }

  // SEO: images in content should have alt text
  const imgRegex = /!\[\s*\]/g;
  const emptyAlts = content.match(imgRegex);
  if (emptyAlts) {
    warnings.push(`${emptyAlts.length} image(s) in content have empty alt text — bad for SEO and accessibility`);
  }

  // SEO: heading hierarchy
  const h1Count = (content.match(/^# /gm) || []).length;
  if (h1Count > 0) {
    warnings.push(`Content contains ${h1Count} H1 heading(s) — the post title is already H1, use H2+ in body`);
  }
